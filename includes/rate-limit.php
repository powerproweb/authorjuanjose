 ...
 */
function ajj_rate_limit_client_ip(): string
{
 ...
 */
function ajj_rate_limit_check(string $bucket, string $identifier, int $maxRequests, int $windowSeconds): array
{
    if ($maxRequests <= 0 || $windowSeconds <= 0) {
        return ['allowed' => true, 'retry_after' => 0];
    }

    $storageDir = dirname(__DIR__) . '/data/rate_limit';
